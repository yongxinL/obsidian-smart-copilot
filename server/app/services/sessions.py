...
async def record_login_attempt(
    ...
    *,
    ...
    Caller (routes/auth.py) opens a dedicated session_with_rls context for
    this call and commits immediately after — ensuring the attempt row is
    durable before the password verify step. We use flush() here (not
    session.begin()) because session_with_rls already starts a transaction
    via the set_config calls; wrapping in begin() would raise
...
async def rotate_refresh(
    ...
    *,
    ...
    Note: session_with_rls opens an auto-begun transaction via set_config;
    wrapping in session.begin() would raise InvalidRequestError. Caller
    commits the session after this call returns.
    """
