...
def _ctx_for(
    user_id: uuid.UUID, *, role: str = "user", request_id: str = "test-rls"
) -> OperationContext:
...
async def test_no_guc_leak_after_request(
    test_engine: AsyncEngine, seed_user_a: uuid.UUID
) -> None:
...
async def test_user_b_sees_empty_guc(
    test_engine: AsyncEngine, seed_user_a: uuid.UUID, seed_user_b: uuid.UUID
) -> None:
...
async def test_cross_user_read_blocked(
    test_engine: AsyncEngine, seed_user_a: uuid.UUID, seed_user_b: uuid.UUID
) -> None:
    ...
    async for sess_b in session_with_rls(_ctx_for(seed_user_b)):
        rows = (
            await sess_b.execute(text("SELECT count(*) FROM provider_keys"))
        ).scalar_one()
        assert rows == 0, f"RLS leak: user B saw {rows} rows from user A"
    # Cleanup as user A
    async for sess_a in session_with_rls(_ctx_for(seed_user_a)):
        await sess_a.execute(
            text("DELETE FROM provider_keys WHERE user_id = :uid"),
            {"uid": str(seed_user_a)},
        )
        await sess_a.commit()
...
async def test_system_context_bypass(test_engine: AsyncEngine) -> None:
    ...
    async for session in session_with_rls(ctx):
        ...
        await session.execute(
            text(
                "DELETE FROM audit_log WHERE action = 'system_test' AND request_id = :rid"
            ),
            {"rid": ctx.request_id},
        )
        await session.commit()
