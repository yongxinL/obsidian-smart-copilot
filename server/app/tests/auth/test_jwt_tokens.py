...
def test_jwt_roundtrip_includes_sub_role_jti() -> None:
    ...
    token = issue_access_jwt(
        user_id=uid, role="admin", signing_key=_SIGNING, ttl_seconds=900
    )
...
def test_decode_rejects_alg_none() -> None:
    ...
    payload = (
        base64.urlsafe_b64encode(
            json.dumps(
                {"sub": "x", "role": "admin", "jti": "j", "exp": 9999999999}
            ).encode()
        )
        .rstrip(b"=")
        .decode()
...
def test_jwt_expiry_enforced() -> None:
    ...
    token = issue_access_jwt(
        user_id=uuid.uuid4(), role="user", signing_key=_SIGNING, ttl_seconds=1
    )
    time.sleep(2)
    with pytest.raises(JWTError):
