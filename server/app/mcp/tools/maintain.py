"""maintain stubs — D-05 not_implemented payload, registered + discoverable.
...
_AVAILABLE_IN_PHASE = "4"

_TOOLS: tuple[tuple[str, str], ...] = (
    (
        "maintain.run",
        "Run a maintenance task (orphan audit, dead-link repair, re-index). "
        "Available in Phase 4.",
    ),
    (
        "maintain.report",
        "Return the latest maintenance report (orphaned pages, broken links, "
        "index statistics). Available in Phase 4.",
    ),
)
